...
import KodlamaIO.HRMS.business.validation.CandidateUserValidatorService;
...
public class CandidateUserManager extends UserManager<CandidateUser> implements CandidateUserService{
	...
	public CandidateUserManager(UserDao<CandidateUser> userDao, CandidateUserValidatorService candidateUserValidatorService,
			UserCheckService userCheckService, EmailService emailService) {
		super(userDao);
	...
	public Result add(CandidateUser candidateUser) {
		...
		results.add(BusinessEngine.run(this.ısUserReal(candidateUser.getIdentityNumber(), candidateUser.getFirstName(), candidateUser.getLastName(), candidateUser.getBirthDate())));
